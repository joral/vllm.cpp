...
`8c4f639c-0ebb-44de-b5e4-0e192143f7c3` (`succeeded`, exit 0) read
`importlib.metadata.version("vllm")` from `cd /` against a source build on
`dgx:gpu0`, and recorded `metadata_vllm='0.28.1rc1.dev132+ge126687a9'` beside an
equal `runtime_vllm___version__` in `/workspace/step6-c1a-2818/versions.txt`
([`sync/2026-09-05-e126687-step6-c1a.md`](sync/2026-09-05-e126687-step6-c1a.md)
§R1). That is the reading [#2818](https://github.com/mudler/vllm.cpp/issues/2818)
item 1 asked a lease to take, and it is what this field now rests on. `rc logs`
for that job returns nothing today — a positive control on the same client still
returns `7386f034`'s 316 lines — so the share artifact and that sync file are its
record, which is why the record exists.
